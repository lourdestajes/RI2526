...
public class Printer {
    ...
    public static void printWorkOrder(WorkOrderDto rep) {
        ...
        Console.printf("\t%s \t%-40.40s \t%td/%<tm/%<tY \t%-12.12s \t%.2f\n", rep.id, rep.description, rep.date,
                rep.state, rep.amount);
    }

    public static void printInvoicingWorkOrder(InvoicingWorkOrderDto rep) {
    ...
    public static void print(SparePartDto sp) {
        Console.printf("\t%-10.10s %-30.30s %4d %4d %4d %6.2f €\n", sp.code, sp.description, sp.stock, sp.minStock,
                sp.maxStock, sp.price);
    }
    ...
    private static void printPayrollLine(PayrollDto p) {
        Console.printf("\t%s \t%-12.12s \t%.2f \t%.2f \t%.2f\n", p.contractId, p.date, p.grossSalary, p.totalDeductions,
                p.netSalary);
    }
    ...
    public static void printSumarizedContract(ContractDto c) {
        Console.printf("\t%s \t%-10.10s \t%-30.30s \t%-12.12s \t%s \t%s \t%s \n", c.id, c.mechanic.nif,
                c.mechanic.name + " " + c.mechanic.surname, c.contractType.name, c.startDate,
                c.endDate != null ? c.endDate : "---", c.state);
    }
