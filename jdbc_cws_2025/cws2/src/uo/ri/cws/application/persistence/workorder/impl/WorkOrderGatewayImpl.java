...
public class WorkOrderGatewayImpl implements WorkOrderGateway {
    ...
    public List<WorkOrderRecord> findByIds(List<String> ids)
            throws PersistenceException {
        List<WorkOrderRecord> result = new ArrayList<>();
        JdbcTemplate<WorkOrderRecord> query = new JdbcTemplate<WorkOrderRecord>()
                                                                                 .forNamedSql(
    ...
    public List<WorkOrderRecord> findNotInvoicedByClientNif(String nif) {
        return new JdbcTemplate<WorkOrderRecord>()
                                                  .forNamedSql(
                                                          "TWORKORDERS_FIND_NOT_INVOICED_BY_CLIENT_NIF")
                                                  .withBinder(
                                                          pst -> pst.setString(
                                                                  1, nif))
                                                  .withRowMapper(
    ...
    public List<WorkOrderRecord> findInvoicedByVehicleId(String id) {
        try {
            ...
            try (PreparedStatement ps = c.prepareStatement(
                    "SELECT * FROM TWORKORDERS WHERE VEHICLE_ID = ? AND STATE = 'INVOICED'")) {
                ps.setString(1, id);
                try (ResultSet rs = ps.executeQuery()) {
                    List<WorkOrderRecord> result = new ArrayList<>();
                    while (rs.next()) {
                        result.add(assembler.toRecord(rs));
                    }
                    return result;
                }
            }
